...
class DispatcherStudio:
    ...
    def _gpio_button_callback(self, pin):
        ...
        if pin == self._takeover_button_pin:
            event = DispatcherButtonEvent(self, DispatcherStudio.BUTTON_TAKEOVER)
        elif pin == self._release_button_pin:
            event = DispatcherButtonEvent(self, DispatcherStudio.BUTTON_RELEASE)
        elif pin == self._immediate_button_pin:
            event = DispatcherButtonEvent(self, DispatcherStudio.BUTTON_IMMEDIATE)
...
class DispatcherButtonEvent:
    def __init__(self, studio: DispatcherStudio, button: str) -> None:
        assert button in DispatcherStudio.BUTTONS
        self._studio = studio
        self._button = button
...
class Dispatcher:
    def __init__(self,
        ...
        studio_mapping = list(studio_mapping) + [
            bermudafunk.DispatcherStudioDefinition(studio=self._automat_studio, selector_value=automat_selector_value)
        ]
    ...
    async def _change_state(self, event: DispatcherButtonEvent):
        ...
        if self._on_air_studio == self._automat_studio:
            ...
            if event.button == DispatcherStudio.BUTTON_TAKEOVER:
                # → nicht „Sofort-Status“
            ...
            elif event.button == DispatcherStudio.BUTTON_RELEASE:
            ...
            elif event.button == DispatcherStudio.BUTTON_IMMEDIATE:
